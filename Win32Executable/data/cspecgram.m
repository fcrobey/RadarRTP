...
% Modified by Frank Robey, 2016 so that it returns a complex rather than
% real output.
##
...
function [S_r, f_r, t_r] = cspecgram(x, n = min(256, length(x)), Fs = 2, window = hanning(n), overlap = ceil(length(window)/2))
  ...
  fshft=[ret_n+1:n 1:ret_n];
  S = S(fshft, :);

  f = [(-ret_n):ret_n-1]*Fs/n;
  t = offset/Fs;
  if nargout==0
